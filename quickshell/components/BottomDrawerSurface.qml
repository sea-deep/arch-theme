...
Shape {
    ...
    ShapePath {
        ...
        // 1. Start at bottom-left outer shoulder along the screen bottom edge
        startX: -root.shoulderRadius
        startY: root.height

        // 2. Bottom-Left Inverted Fillet (concave curve from bottom screen edge up into left wall)
        PathQuad {
            controlX: 0
        ...
        // 8. Bottom-Right Inverted Fillet (concave curve from right wall out to bottom screen edge)
        PathQuad {
            controlX: root.width
            controlY: root.height

...
# Monolithic Theme Synchronizer for sea-deep Theme
...
write_gtk_colors() {
    ...
    mkdir -p "$(dirname "$target")"
    cat << EOF > "$target.tmp"
/* Monolithic sea-deep GTK Colors — Generated by sync-theme.sh */
...
if command -v gtk-update-icon-cache >/dev/null 2>&1; then
    ...
    gtk-update-icon-cache -q -f -t "$HOME/.local/share/icons/sea-deep" 2>/dev/null || true
fi

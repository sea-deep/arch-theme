...
if pgrep -x wf-recorder > /dev/null; then
    ...
    case $chosen in
        "Full Screen")
            wf-recorder --audio="$(pactl get-default-sink).monitor" -f "$FILENAME" -p crf=15 -x yuv420p > /tmp/wf_recorder.log 2>&1 &
            ;;
        "Selected Region"|"Specific Window")
            ...
            if [ -n "$GEOMETRY" ]; then
                ...
                wf-recorder --audio="$(pactl get-default-sink).monitor" -g "$GEOMETRY" -f "$FILENAME" -p crf=15 -x yuv420p > /tmp/wf_recorder.log 2>&1 &
            else
